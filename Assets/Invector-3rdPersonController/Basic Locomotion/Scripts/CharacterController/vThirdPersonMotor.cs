...
namespace Invector.vCharacterController
{
    public class vThirdPersonMotor : vCharacter
    {
        ...
        {
            if (isGrounded || verticalVelocity > fallMinVerticalVelocity || !_canApplyFallDamage || fallMinHeight == 0)
            {
                return;
            }
        ...
        private void ControlMaterialPhysics()
        {
            ...
            if (isGrounded && input == Vector3.zero && !isSliding)
            {
                _capsuleCollider.material = maxFrictionPhysics;
            }
            else if (isGrounded && input != Vector3.zero && !isSliding)
            {
